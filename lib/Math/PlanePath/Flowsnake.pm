...
# Jeffrey Ventrella
# root-7 family
# "inner-flip" which is initial state reversal
...
For example N=9079 is base-7 "35320" so a single low 0 for zcount=1 and
...
have their sub-parts "plain" in all cases, both the plain and reverse forms.
...
non-zero such as "5000" then it's taken as zdigit=5 and above that for the
plain/reverse a tdigit=0 is then assumed.  The first turn is at N=1 so
there's always at least one non-0 for the zdigit.
