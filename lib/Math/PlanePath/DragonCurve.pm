...
For example N=12 is binary 0b1100, the lowest 1 bit is the 0b.1.. and the
bit above that is a 1, which means turn to the right.  Or later N=18 is
binary 0b10010, the lowest 1 is 0b...1. and the bit above that is 0, so turn
left there.
...
above that is a 0 too, so after going to N=13 the turn there at 13 is to the
left.  Or for N=18=0b10010 the lowest 0 is again the least significant bit,
but above it is a 1, so at N=19 the turn there is to the right.

This too can be found with some bit twiddling, either with a ones-complement
to turn it into low ones, or directly as for example
...
and 1, or 1 and 2, and the extras A038189 and A082410 differ only in
beginning with an extra initial 0.

The square numbering A126937 is a Dragon curve going downwards towards
negative Y and the spiral numbering starting from N=0 and spiralling upwards
towards positive Y.  So starting i=0 it can be calculated as
