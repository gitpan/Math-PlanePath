...
The default k=3 visits X,Y with one odd one even and perhaps a common factor
3^m.
...
middle, when k is odd there's just one.  The resulting tree for example with
k=4 is
...
base-k.  For example k=10 with N in decimal,
