...
sub path_triangular_xyxy_is_left_boundary {
  ...
  ### path_triangular_xyxy_is_left_boundary() ...

  my $n = path_xyxy_to_n ($path, $x1,$y1, $x2,$y2);
  my $n_hi;
  if (defined $level) {
    if ($level < 0) {
  ...
  my $y3 = $y1 + ($dy+$dx)/2;
  ### points: "$x1,$y1 to $x2,$y2 and left $x3,$y2"

  foreach my $n1 (path_xyxy_to_n ($path, $x1,$y1, $x3,$y3),
                  path_xyxy_to_n ($path, $x2,$y2, $x3,$y3)) {
    ### $n1
    if (! defined $n1) {
...
sub path_triangular_xyxy_is_right_boundary {
  ...
  return (! MyOEIS::path_xyxy_is_traversed ($path, $x1,$y1, $x3,$y3)
          || ! MyOEIS::path_xyxy_is_traversed ($path, $x2,$y2, $x3,$y3));
}
