...
sub print_table {
  ...
  foreach my $i (0 .. $#$aref) {
    ...
    foreach my $rev (0, 1) {
      foreach my $rot (0, 1, 2, 3) {
        foreach my $orig_digit (0 .. 24) {
          ...
          my $shift_yo = $yo - (($xo % 5) >= 3);
          my $shift_xo = $xo - (($shift_yo % 5) == 1 || ($shift_yo % 5) == 2);
          if ($shift_xo < 0 || $shift_xo > 4 || $shift_yo < 0 || $shift_yo > 4) {
            die "oops rot=$rot xo=$xo yo=$yo  shiftxo=$shift_xo shiftyo=$shift_yo";
          }
          if ($rot & 2) {
            $shift_xo = 4 - $shift_xo;
            $shift_yo = 4 - $shift_yo;
          ...
          if ($rot & 1) {
            ($shift_xo,$shift_yo) = (4-$shift_yo,$shift_xo);
          }
    ...
    print_table ("last_digit_to_x", \@digit_to_x);
    print_table ("last_digit_to_y", \@digit_to_y);
